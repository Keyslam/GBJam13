...
export class Sprite extends Component {
    ...
    private draw(event: DrawEvent): void {
        const height = this.entity.tryGetComponent(Height);

        const y = this.position.y - (height?.value ?? 0);

        height?.draw(this.image);
        this.renderService.drawImage(this.image, this.quad, this.position.x, y, this.position.z, this.flipped, this.flash);
